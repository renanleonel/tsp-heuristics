...
float verificador(float **vertex, int a, int b, int size)
{
    int a2;
    if (a == 0)
    {
...
void vizinhoMaisProx(float **vertex, int size, float **path)
{
    int tam = 0;
...
void troca_simples(float **tour, int i, int k, int size)
{
    for (; i < k; i++, k--)
    {
        float aux1, aux2;
    ...
    while (improve)
    {
        ...
        for (int i = 0; i < size; i++)
        {
            ...
            if (best_diference < 0)
            {
                ...
                improve = true;
                out << fixed << custo(v, size) << " " << (clock() - tempo_inicial) / (double)CLOCKS_PER_SEC << "\n";
            }
        }
    }
}
...
int main()
{
    ...
    float **pathVizinhoMaisProximo = (float **)malloc(tam * sizeof(float *));
    for (int i = 0; i < tam; i++)
    {
        pathVizinhoMaisProximo[i] = (float *)malloc(2 * sizeof(float));
    }
