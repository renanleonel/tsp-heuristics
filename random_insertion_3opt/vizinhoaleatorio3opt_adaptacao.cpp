...
void vizinhoAleatorio(float vertex[][2], int size, float path[][2])
{
...
float melhor_troca(float aux[][2], int i, int j, int k, int size)
{
    float aX, aY;
    aX = aux[i - 1][0];
    aY = aux[i - 1][1];
...
void troca_simples(float aux[][2], int ini, int fim)
{
    float aux1;
    float aux2;
...
// 3opt implementado com uma adaptacao do first improvement, onde eh utilizado
// um numero de tentatativas de melhoramento ao inves de para apenas com uma melhora
void threeOpt(int size, float tour[][2])
{
    int improve = 0;
    int best_i;
    int best_k;
...
int main()
{
    ...
    for (int i = 0; i < tam; i++)
    {
        entrada[i][0] = vertex[i].first;
        entrada[i][1] = vertex[i].second;
    }
    float pathVizinhoAleatorio[tam][2];
    // Constuindo a caminho
    vizinhoAleatorio(entrada, tam, pathVizinhoAleatorio);
